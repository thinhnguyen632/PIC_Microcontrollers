...
byte MFRC522_Write(byte addr,byte *pData)
{
    byte status;
    unsigned int16  Length;
    byte i,Buffer[MAX_LEN]; 
    
    Buffer[0] = MF1_WRITE;
    Buffer[1] = addr;
    calculateCRC(Buffer,2,&Buffer[2]);
 
    status = commandTag(MFRC522_TRANSCEIVE,Buffer,4,Buffer,&Length);
    ...
    if (status == MI_OK)
    {
        ...
        status = commandTag(MFRC522_TRANSCEIVE,Buffer,18,Buffer,&Length);
        if ((status != MI_OK) || (Length != 4) || ((Buffer[0] & 0x0F) != 0x0A))
        {   status = MI_ERR;   }
    }
...
byte MFRC522_Read(byte addr,byte *pData)
{
    byte status;
    unsigned int16  Length;
    byte i,Buffer[MAX_LEN]; 
